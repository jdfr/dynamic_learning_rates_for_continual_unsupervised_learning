...
NomFichResultados = 'ResultadosSegmentacionGHNF';
...
%             NumPasos = NumEpocas*NumMuestras;
            ...
            fprintf('------------------------------------\n');            
            tic;
            [Modelo] = TrainGHNF(Muestras,NumEpocas,MaxNeurons,Tau,Lambda,EpsilonB,EpsilonN,Alpha,AMax,D,1);
            CpuTime = toc;
            if isempty(Modelo),
                continue;
            end
        ...
        if (isempty(Results{NdxColorSpace,NdxVideo})),
            % Test GHNF
            [Winners,Errors] = TestGHNF(GetCentroidsGHNF(Modelo),Modelo.Samples);
