function DemoCompetitiveCORA(cora, prefixpath, numRepeats, numfactors, pre_load, do_load, do_show, trainingType, netType, trainMode, mqeMode, slidingWindow, initial_LearningRate, MaxLearningRate, NumSamples);
    ...
    for a=1:length(experiments)
      for i=1:size(experiments{a}.meanQuantizationErrors,1)
        for j=1:size(experiments{a}.meanQuantizationErrors,2)
          ...
          %prototypes  = cell(numRepeats,1);
          %connections = cell(numRepeats,1);
          parfor b=1:numRepeats
            ...
            [meanQuantizationErrors(b), ~, winners] = compute_consensus_quantization_error(mqeMode, Model.Prototypes, originalSamples);
            %prototypes{b} = Model.Prototypes;
            %if isGNG
            %  connections{b} = Model.Connections;
            %end
            metrics = computeMetrics(cora.paperClass, Model, NumClusters, winners, centroids, originalSamples, originalSamplesTranposed, dissimMatrix, isCompetitive, isSOM, isGNG);
          ...
          for b=1:numRepeats
            ...
            experiments{a}.accuracy(i,j,b)         = runs{b}{5}(6);
            if experiments{a}.meanQuantizationErrors(i,j,b) < best_run.mqe(a)
              best_run.mqe(a)                           = experiments{a}.meanQuantizationErrors(i,j,b);
              %best_run.receptiveFieldSizes{a}           = experiments{a}.receptiveFieldSizes{i,j,b};
