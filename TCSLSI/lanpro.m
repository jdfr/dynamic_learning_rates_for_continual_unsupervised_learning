...
if nargin<1, error('В lanpro недостаточно входных парамеров');  end
if isnumeric(A) | isstruct(A)
  if isnumeric(A)
    [m n] = size(A);
    if m~=n | ~isequal(A,A') | ~isreal(A)
      error('В lanpro матрица A должна быть действительной и симметричной')
    end  
  elseif isstruct(A)
    [m n] = size(A.L);
  end
  ...
  if nargin<2
    error('В lanpro недостаточно входных параметров');
  end
...
if isempty(Q_k) % New Lanczos tridiagonalization.
  ...
  % Allocate space
  Q_k = [Q_k zeros(floor(n), floor(kmax-j))]; 
  alpha = zeros(kmax+1,1);  beta = zeros(kmax+1,1);
  alpha(1:j) = diag(T_k);  
  if j>1
    beta(2:j) = diag(T_k,-1);
