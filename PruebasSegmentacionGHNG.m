...
NomFichResultados = 'ResultadosSegmentacionGHNG';
...
%             NumPasos = NumEpocas*NumMuestras;
            ...
            fprintf('------------------------------------\n');            
            tic;
            [Modelo] = TrainGHNG(Muestras,NumEpocas,MaxNeurons,Tau,Lambda,EpsilonB,EpsilonN,Alpha,AMax,D,1);
            CpuTime = toc;
            if isempty(Modelo),
                continue;
            end
        ...
        if (isempty(Results{NdxColorSpace,NdxVideo})),
            % Test GHNF
            [Winners,Errors] = TestGHNG(GetCentroidsGHNG(Modelo),Modelo.Samples);
            ...
            PerformanceMeasures = TestGHNFSegmentation(Modelo,Winners,Errors,...
                MyColorSpace,MyVideo,NumFrames(NdxVideo),NumGTFrames(NdxVideo),...
                DeltaFrames(NdxVideo),PathMyVideo,PathMyGT,Mu,Features,WindowSizeRGBMedian,MaxNeurons); 
